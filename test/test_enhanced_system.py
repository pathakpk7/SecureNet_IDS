...
try:
    from backend.ml import MLPredictor, cicids_feature_extractor
    from backend.database import db_manager as enhanced_db_manager
...
async def test_database_integration():
    ...
    print("✅ Database manager operational")
    ...
    if logs:
        print(f"   Latest Log: {logs[0].get('source_ip', 'N/A')} -> {logs[0].get('destination_ip', 'N/A')}")
...
async def test_threat_intelligence():
    ...
    try:
        ...
        for result in threat_results:
            res_dict = result.dict() if hasattr(result, 'dict') else (vars(result) if not isinstance(result, dict) else result)
            print(f"   Source: {res_dict.get('source', res_dict.get('sources', 'N/A'))}")
            print(f"   Malicious: {res_dict.get('malicious', False)}")
            print(f"   Confidence: {res_dict.get('confidence', 0):.4f}")
            print(f"   Type: {res_dict.get('type', res_dict.get('attack_type', 'N/A'))}")
            print(f"   Details: {len(res_dict.get('details', {}))} items")
            print()
        
        # Analyze combined results
        if hasattr(threat_intel_manager, 'analyze_threat_intel'):
        ...
        print(f"⚠️ Threat intelligence test notice: {str(e)}")
