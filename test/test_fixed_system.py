...
try:
    from backend.ml import fixed_ml_predictor, FixedMLPredictor, fixed_feature_extractor, FixedFeatureExtractor
    from backend.database import fixed_db_manager, FixedDatabaseManager
...
async def test_fixed_feature_extraction():
    ...
    for flow in active_flows[:3]:
        ...
        print(f"     TCP Flags: {flow.get('tcp_flags', 'N/A')}")
...
async def test_fixed_database():
    ...
    health_status = "healthy" if db_health else "in-memory-mode"
