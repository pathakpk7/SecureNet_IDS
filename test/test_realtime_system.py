#!/usr/bin/env python3
"""
SecureNet IDS - Real-time System Test
Comprehensive testing of the real-time intrusion detection system.
"""
...
try:
    from backend.ml import MLPredictor, realtime_feature_extractor, RealTimeFeatureExtractor
    from backend.database import enhanced_db_manager
    from backend.threat_intelligence import threat_intel_manager
    from backend.capture import realtime_capture, RealTimePacketCapture
...
class MockPacketGenerator:
    """Generate mock packets for testing real-time processing."""
    
    def __init__(self):
        self.packet_counter = 0
    
    def generate_normal_packet(self) -> PacketData:
        """Generate a normal packet for testing."""
    ...
    def generate_attack_packet(self) -> PacketData:
        """Generate an attack packet for testing."""
        self.packet_counter += 1
        
        # Simulate different attack types
        attack_types = [
            {'src': "10.0.0.100", 'dst': "192.168.1.1", 'size': 50000, 'port': 80},  # DoS
            {'src': "172.16.0.50", 'dst': "192.168.1.1", 'size': 64, 'port': 22},   # Port scan
            {'src': "203.0.113.10", 'dst': "192.168.1.1", 'size': 1500, 'port': 443}, # Suspicious
        ]
...
async def test_ml_model_realtime():
    """Test the ML model with real-time feature extraction."""
    print("\n🤖 Testing Real-time ML Model...")
    ...
    # Test with real-time feature extraction
    packet_generator = MockPacketGenerator()
    
    # Test normal packet
    normal_packet = packet_generator.generate_normal_packet()
    features = realtime_feature_extractor.extract_cicids_features(normal_packet)
    ...
    attack_features = realtime_feature_extractor.extract_cicids_features(attack_packet)
...
async def test_realtime_feature_extraction():
    """Test real-time feature extraction with flow tracking."""
    print("\n🔧 Testing Real-time Feature Extraction...")
    
    packet_generator = MockPacketGenerator()
    
    # Generate multiple packets for the same flow
    print("📦 Testing Flow Tracking (multiple packets in same flow)...")
    ...
    for i in range(5):
        packet = PacketData(
            ...
            tcp_flags={'syn': True if i == 0 else False, 'ack': True},
    ...
    for i, packet in enumerate(flow_packets, 1):
        features = realtime_feature_extractor.extract_cicids_features(packet)
        print(f"   Packet {i}:")
    ...
    # Get flow statistics
    flow_stats = realtime_feature_extractor.get_flow_statistics()
    print(f"\n📈 Flow Statistics:")
    ...
    # Get active flows
    active_flows = realtime_feature_extractor.get_active_flows(limit=5)
...
async def test_realtime_database():
    """Test real-time database operations."""
    print("\n💾 Testing Real-time Database Operations...")
    
    is_conn = enhanced_db_manager.is_connected()
    ...
    health_status = "healthy" if health else "in-memory-mode"
    ...
    # Extract features
    features = realtime_feature_extractor.extract_cicids_features(attack_packet)
    ...
    if detection_data['prediction']:
        alert_data = {
            "id": f"realtime_alert_{int(time.time())}",
...
async def test_realtime_threat_intel():
    """Test real-time threat intelligence integration."""
    print("\n🕵️ Testing Real-time Threat Intelligence...")
    ...
    try:
        ...
        print(f"📊 Real-time Threat Intelligence Results:")
        ...
        print(f"⚠️ Real-time threat intelligence test failed: {str(e)}")
...
async def test_realtime_pipeline():
    ...
    print("\n🔄 Testing Complete Real-time Pipeline...")
    ...
    if enhanced_db_manager.is_connected():
        ...
        attack_log_stored = await enhanced_db_manager.store_detection_log(attack_detection_data)
        print(f"   Attack Log Storage: {'✅' if attack_log_stored else '❌'}")
        
        # Store alert
        alert_data = {
            "id": f"pipeline_alert_{int(time.time())}",
...
def check_realtime_requirements():
    """Check real-time system requirements."""
    print("\n🔧 Checking Real-time System Requirements...")
    ...
    for dep in db_deps:
        try:
            ...
            print(f"   ⚠️ {dep} (MISSING - some features limited)")
...
async def main():
    """Main real-time test function."""
    
    display_realtime_info()
    
    # Check requirements
    if not check_realtime_requirements():
        print("\n❌ Real-time system requirements not met!")
        print("Please ensure all dependencies are available.")
        return 1
    
    print("\n✅ Real-time system requirements check passed!")
    
    try:
        # Run real-time tests
        tests = [
            ("Real-time ML Model", test_ml_model_realtime),
            ("Real-time Feature Extraction", test_realtime_feature_extraction),
            ("Real-time Database", test_realtime_database),
            ("Real-time Threat Intelligence", test_realtime_threat_intel),
            ("Complete Real-time Pipeline", test_realtime_pipeline)
        ]
        ...
        print("\n⏹ Real-time tests interrupted by user")
        return 1
    except Exception as e:
