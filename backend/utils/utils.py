...
validate_ip = validate_ip_address
...
def create_response(success: bool = True, message: str = "", data: Any = None, error: str = None) -> Dict[str, Any]:
    """
...
def export_data_to_csv(data: List[Dict[str, Any]], filename: Optional[str] = None, fieldnames: Optional[List[str]] = None) -> str:
    """
    Export data to CSV format (returns CSV string and optionally writes to file).
    ...
    output = io.StringIO()
    if not data:
        output.write("id,timestamp,source_ip,destination_ip,protocol,attack_type,risk_level,confidence\n")
